...
def run_uninstall(harnesses: Sequence[str], *, yes: bool, dry_run: bool,
    ...
    if purge_data:
        exit_code = _purge_data(yes=yes, dry_run=dry_run, input_fn=input_fn,
                                stdout=stdout, env=env, home=home,
                                root_override=root)
        if exit_code != 0:
            return exit_code
...
def _purge_data(*, yes: bool, dry_run: bool, input_fn: Callable[[str], str],
                stdout: TextIO, env: Mapping[str, str], home: Path,
    ...
    outranks both.
    """
    root = _resolve_storage_root(root_override, env, home)
    stdout.write(f"\nmemory storage root: {root}\n")
    if root.is_symlink():
        stdout.write(
            f"memriver uninstall: {root} is a symlink; memriver will not purge "
    ...
    if not root.is_dir():
        stdout.write(
            f"memriver uninstall: {root} is not a directory; nothing was removed.\n"
        )
        return 1
    if dry_run:
    ...
    if not yes:
        try:
            answer = input_fn(f"remove the entire memory store at {root}? [y/N] ")
        except EOFError:
            stdout.write(
    ...
    try:
        ...
        stdout.write(
            f"memriver uninstall: {root} was only partly removed ({reason}); the "
            "harness configuration above was removed successfully. Delete the "
            "remaining directory by hand.\n"
        )
        return 1
    stdout.write(f"removed {root}\n")
    return 0
