...
class MemoryStore(Protocol):
    ...
    - Authorization is decided on the entry's stored `project_id` at the
      ...
      section. `ctx.readable()`/`ctx.writable()` are the only inputs.
    - `record`: a global target raises `GlobalReadOnly`; a target outside
      `ctx.writable()`, or a project that does not exist, raises
...
class ProjectStore(Protocol):
    ...
    - `search`: exactly one project. A project outside `ctx.readable()` or one
      that does not exist answers `[]`; a damaged project file or an unusable
      `memories/` is `StorageFailure`; a single damaged memory is skipped. `query=None` lists everything,
