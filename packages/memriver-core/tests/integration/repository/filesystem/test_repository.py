...
@pytest.fixture
def repo(root) -> FileMemoryRepository:
...
def test_signatures_match_the_repository_protocol(repo):
    for name in MemoryRepository.__protocol_attrs__:
        ...
        assert inspect.signature(getattr(repo, name)) == port, name


# --- storage layout ---

def test_create_writes_into_the_global_entries_directory(repo, root):
    m = _m()
    repo.create(m, CTX)
    path = _entry_path(root, m)
    assert path.exists()
    assert repo.get(m.id, CTX) == m


def test_project_scope_gets_its_own_directory(repo, root):
    m = _m(scope=Scope.project(ProjectId("demo-abc123")))
    repo.create(m, AccessContext(project_id=ProjectId("demo-abc123")))
...
def test_create_rejects_path_traversal_id(repo, root):
    ...
    with pytest.raises(ValueError):
        repo.create(_m(id="../../../outside"), CTX)
    assert list(root.glob("**/*.md")) == []


def test_get_rejects_malformed_ids(repo):
...
def test_lookup_accepts_slug_and_ulid_shapes(repo):
    repo.create(_m(body="a", type="user", id="my-slug"), CTX)
    ulid_memory = _m(body="b", type="user")
    repo.create(ulid_memory, CTX)
    assert repo.get("my-slug", CTX).body == "a"
    assert repo.get(ulid_memory.id, CTX).body == "b"
    with pytest.raises(MemoryNotFound):
        repo.get("../escape", CTX)
    with pytest.raises(MemoryNotFound):
        repo.get("Bad_Name", CTX)


# --- directory is truth ---

def test_get_refuses_memory_whose_frontmatter_scope_contradicts_its_directory(repo,
                                                                             root):
...
def test_iter_visible_skips_a_scope_mismatched_file(repo, root):
...
def test_iter_visible_skips_an_id_mismatched_file(repo, root):
    mine = _m(body="mine stays visible")
...
def test_iter_visible_skips_an_undecodable_file(repo, root):
...
def test_update_body_refuses_when_id_contradicts_filename(repo, root):
    mine = _m(body="original body")
    repo.create(mine, CTX)
    path = _entry_path(root, mine)
    mine.id = "bar"
    ...
    with pytest.raises(MemoryNotFound):
        repo.update_body(path.stem, "hijacked", CTX)
    assert not (path.parent / "bar.md").exists()
    assert path.read_text(encoding="utf-8") == encode(mine)


def test_delete_refuses_a_hand_written_non_entry_file(repo, root):
...
def test_create_refuses_to_clobber_a_hand_written_non_entry_file(repo, root):
    ...
    before = path.read_bytes()
    with pytest.raises(UnreadableMemory):
        repo.create(_m(body="v1", type="user", id="notes"), CTX)
    assert path.read_bytes() == before


def test_create_refuses_clobber_when_name_equals_missing_frontmatter_key(repo, root):
    ...
    before = path.read_bytes()
    with pytest.raises(UnreadableMemory):
        repo.create(_m(body="v1", type="user", id="source"), CTX)
    assert path.read_bytes() == before


def test_create_refuses_when_name_taken_by_scope_mismatched_file(repo, root):
...
def test_create_refuses_when_name_taken_by_id_mismatched_file(repo, root):
...
def test_project_create_ignores_a_foreign_projects_name(repo, root):
...
def test_create_failure_raises_an_opaque_storage_failure(repo, root):
...
def test_read_failure_raises_an_opaque_storage_failure(repo, root):
...
def test_delete_failure_raises_an_opaque_storage_failure(repo, root, monkeypatch):
    repo.create(_m(id="n"), CTX)
