...
def world(tmp_path):
    ...
    ctx = AccessContext(project_id=project.id, global_project_id=global_id)
    return root, memory_store, project_store, project.id, global_id, ctx
...
def test_layout_is_flat_memories_project_files_and_a_manifest(world):
    root, memory_store, _, project_id, global_id, ctx = world
...
def test_a_symlinked_memory_file_is_never_followed_and_reads_as_damage(world, tmp_path):
    ...
    with pytest.raises(StorageFailure):
        memory_store.read(memory.id, ctx)
...
def test_a_fifo_at_a_memory_name_is_refused_without_blocking(world):
    root, memory_store, project_store, project_id, _, ctx = world
    memory_id = new_id()
    (root / "memories").mkdir(exist_ok=True)
    fifo = root / "memories" / f"{memory_id}.md"
    os.mkfifo(fifo)
    with pytest.raises(StorageFailure):
        _call_with_timeout(memory_store.read, memory_id, ctx, unblock=fifo)
    # a scan skips the one damaged file rather than failing
    assert _call_with_timeout(lambda: project_store.search(project_id, ctx, query=None,
                                                           limit=None), unblock=fifo) == []


def test_an_undecodable_memory_file_is_damage_not_absence(world):
    root, memory_store, _, _, _, ctx = world
    memory_id = new_id()
    (root / "memories").mkdir(exist_ok=True)
    (root / "memories" / f"{memory_id}.md").write_text("hand notes\n", encoding="utf-8")
    for action in (lambda: memory_store.read(memory_id, ctx),
                   lambda: memory_store.update(memory_id, ctx, body="x", description=None),
                   lambda: memory_store.delete(memory_id, ctx)):
        with pytest.raises(StorageFailure):
            action()
...
def test_a_write_failure_on_a_located_memory_is_a_storage_failure(world, monkeypatch):
    _, memory_store, _, project_id, _, ctx = world
    memory = _m(project_id)
    memory_store.record(memory, ctx)
...
def test_a_symlinked_data_directory_is_never_followed_for_writes(tmp_path, dirname):
    ...
    if dirname == "projects":
        ...
        ctx = AccessContext(project_id=project.id, global_project_id=global_id)
        with pytest.raises(StorageFailure):
            memory_store.record(_m(project.id), ctx)
...
def test_a_symlinked_data_directory_is_never_followed_for_reads(tmp_path):
    ...
    with pytest.raises(StorageFailure):
        memory_store.read(memory.id, ctx)
    with pytest.raises(StorageFailure):
        project_store.search(project.id, ctx, query=None, limit=None)
...
def test_a_cross_project_scan_never_logs_another_projects_raw_fields(world, caplog):
    root, _, project_store, project_id, _, ctx = world
