#!/usr/bin/env bash
# Host driver for stage 5. Run deliberately: it makes real `codex exec` calls
...
echo "==> running stage5.sh inside the container (codex on Azure AI Foundry; credentials passed by name only)"
docker run --rm \
    -v "$REPO_ROOT/e2e/wheels":/wheels:ro \
    -v "$REPO_ROOT/e2e":/e2e:ro \
    -e AZURE_FOUNDRY_BASEURL \
    -e AZURE_FOUNDRY_API_KEY \
    -e AZURE_FOUNDRY_GPT_DEPLOYMENT \
