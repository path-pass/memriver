#!/usr/bin/env bash
# Host driver for stage 3. Run deliberately: it makes real `claude -p` calls
...
echo "==> running stage3.sh inside the container (Claude Code on Azure AI Foundry; credentials passed by name only)"
docker run --rm \
    ...
    memriver-e2e bash /e2e/stage3.sh
